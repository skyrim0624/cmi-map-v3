...
test('动物候选主展示使用中文名', () => {
  assert.match(source, /const fallbackChineseNames/);
  assert.match(source, /dog: '家犬'/);
  assert.match(source, /cat: '家猫'/);
  ...
  assert.doesNotMatch(source, /formatAnimalCandidateLabel = \(candidate: AnimalIdentificationCandidate\) =>\s+getAnimalScientificName\(candidate\)/);
