...
test('生物识别不再接入 Cloudflare 开源物种模型', () => {
...
test('专业物种识别优先 Gemini 2.5 Flash，并在额度失败时走自托管兜底', () => {
  assert.match(source, /if \(!env\.GOOGLE_AI_STUDIO_API_KEY && !getSelfHostedSpeciesModelUrl\(env\)\)/);
  assert.match(source, /runGeminiSpeciesModel\(env\.GOOGLE_AI_STUDIO_API_KEY, imageBytes, mimeType\)/);
  assert.match(source, /identifyWithSelfHostedSpeciesModel/);
  assert.match(source, /getSelfHostedSpeciesModelUrl/);
  assert.match(source, /getSelfHostedSpeciesModelToken/);
...
test('没有 Gemini 和自托管配置时接口明确不可用', () => {
  assert.match(source, /if \(!env\.GOOGLE_AI_STUDIO_API_KEY && !getSelfHostedSpeciesModelUrl\(env\)\)/);
  assert.match(source, /status: 'unavailable'/);
  assert.match(source, /provider: SELF_HOSTED_SPECIES_MODEL_ID/);
