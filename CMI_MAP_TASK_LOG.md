...
43. [完成] `/mark` 相机生物识别默认关闭，改为用户手动开启
44. [完成] 修复动态页固定 60 条截断导致历史动态看起来丢失
45. [完成] 修复个人页默认打开空图鉴导致旧打卡看起来丢失
46. [完成] 主地图用户打卡点位恢复显示并部署正式域名
47. [完成] 恢复主地图左上角“神奇动物在哪里”固定活动入口

## 执行记录

### 2026-07-04 主地图用户打卡点位恢复显示并部署

- 背景：用户反馈地图上没有大家打卡的点位，随后确认 `cmimap.com` 正式域名仍显示旧状态。
- 根因：V3 主地图对用户推荐点位加了近 7 天过滤，历史打卡被前端隐藏；同时本轮修复推到 GitHub 后，Cloudflare Pages Production 仍停在旧 Source `9c06507`，正式域名没有自动部署到新提交。
- 改动与部署：移除主地图点位近 7 天过滤，保留当前分类 / 搜索筛选；修复提交为 `055b7f8`。随后手动部署 `dist` 到 Cloudflare Pages 项目 `cmi-map` 的 Production，部署地址为 `https://b0f88f8d.cmi-map.pages.dev`，正式域名 `https://cmimap.com/` 已切到 Source `055b7f8`。
- 验证：生产构建和 PWA precache 检查通过；正式域名入口包为 `index-Cb78-T-B.js`，地图包 `CmiMapV3Prototype-BqMW6WJf.js` 不再包含近 7 天过滤；移动端线上复查显示 14 个单点、15 个聚合点、50 个 Leaflet marker 元素，无 framework overlay。

### 2026-06-27 个人页旧打卡可见性修复

- 背景：用户反馈“以前的记录咋都没了”。
- 根因：线上旧打卡数据仍在，同一用户旧记录可查到；6 月 16 日个人页改为神奇动物主题后默认打开“神奇生物图鉴”，非动物打卡用户会先看到空图鉴，误以为历史打卡丢失。
- 改动：个人页默认 tab 改回“我的痕迹”，保留图鉴 tab 但不再作为默认首屏。
- 验证：先新增回归测试并确认失败，再修复默认 tab 后 `node --test src/pages/Profile.test.ts` 通过。

### 2026-06-27 动态页历史动态截断修复

- 背景：用户反馈动态历史有丢失。
- 根因：线上 `recommendations` 表实际有 267 条，最早记录仍在 2026-04-15；动态页合并推荐动态和论坛帖后固定 `.slice(0, 60)`，导致 60 条之后的历史动态被前端隐藏。
- 改动：移除动态页固定 60 条截断，保留按 `created_at` 排序后的完整动态数组；新增回归测试防止再次引入固定条数截断。
- 验证：先新增测试并确认失败，再移除截断后 `node --test src/pages/CmiMapV3Prototype.test.ts` 通过。

### 2026-06-25 相机生物识别改为手动开启
...
### 2026-07-14 首页动态栏高度回退

- 用户确认活动卡已显示，但不希望首页动态栏占用更大的默认地图区域。
- 将折叠态高度恢复为原来的 `96–104px`；完整历史活动仍保留在上拉展开后的动态栏中。

### 2026-07-18 恢复神奇动物固定活动入口

- 背景：用户发现主地图左上角原有的“神奇动物在哪里”入口消失。
- 根因：固定入口错误地从 `upcomingCommunityEvents` 查找活动；该活动在 2026-06-21 结束后被过期过滤，导致入口不再渲染。
- 改动：固定入口改为从完整社区活动列表查找，并保留本地活动数据兜底；地图历史活动 marker 的过期过滤保持不变。
